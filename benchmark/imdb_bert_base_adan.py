...
def process(log_name, trainer, args):
    ...
    handler = CSVHandler("./benchmark/energy/"+ log_name + '_Energy_Results.csv')
    ...
    with open('./benchmark/metrics/'+log_name+'.txt', 'w') as f:
    ...
    with open('./benchmark/loss_val/'+log_name+'_loss.txt', 'w') as f:
        for item in loss:
            f.write(str(item))
            f.write('\n')
    ...
    with open('./benchmark/acc_val/'+log_name+'_acc.txt', 'w') as f:
    ...
    plt.savefig("./benchmark/figure/"+log_name+".png")
...
if __name__ == '__main__':
    ...
    adan_fused_trainer = model_and_trainer(train_loader, test_loader, eval_loader, args)
    args.log_file_name = "imdb_adan_fused_lr1e-4_wd1e-2_wm50_ep2_acc90"
    process(args.log_file_name, adan_fused_trainer, args)